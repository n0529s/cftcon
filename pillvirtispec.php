...
// $floor_num_total ="";
  ...
  while( $result4 = $stmt->fetch(PDO::FETCH_ASSOC)){ 
      ...
      if($pill_sign_c != $pill_sign_b ){
        ...
        $view_vspec4 .= '<td id="aa"><a href=pill_aa.php?id='.$result4['pill_sign'].'>削除</td>';
      }
...
// 杭符号情報抽出2
$stmt = $pdo->prepare("select * from pillvirtispec where gen_name = :gen_name AND floor_num = :floor_num");
$stmt->bindValue(':gen_name', $gen_name, PDO::PARAM_STR);
